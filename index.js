...
export default class Drawer extends Component {
  ...
  componentWillMount() {
    ...
    if (this.props.openDrawerThreshold && process.env.NODE_ENV !== "production")
      console.error(
        "react-native-drawer: openDrawerThreshold is obsolete. Use panThreshold instead."
      );
    if (
      this.props.panStartCompensation &&
      process.env.NODE_ENV !== "production"
    )
      console.error("react-native-drawer: panStartCompensation is deprecated.");
    if (this.props.relativeDrag && process.env.NODE_ENV !== "production")
      console.error("react-native-drawer: relativeDrag is deprecated.");
  ...
  initialize = props => {
    ...
    if (typeof this.props.isScale !== "undefined") {
      styles.main = Object.assign(
        ...
        this.isLeftOrRightSide() ? { top: 0 } : { left: 0 },
    ...
    if (props.initializeOpen || props.open) {
      ...
      if (props.type === "static")
        styles.main[this.props.side] = fullLength - this._offsetOpen;
      ...
      if (props.type === "overlay")
        styles.drawer[this.props.side] =
          this._offsetClosed + this._offsetOpen - fullLength;
      if (props.type === "displace")
        styles.drawer[this.props.side] =
          -fullLength + this._offsetClosed + this._offsetOpen;
    }
  ...
  updatePosition = () => {
    ...
    let ratio =
      (this._length - this._offsetClosed) /
      (this.getOpenLength() - this._offsetClosed);
    ...
    switch (this.props.type) {
      case "overlay":
        drawerProps[this.props.side] = Math.round(
          -this.getDeviceLength() + this._offsetOpen + this._length
        );
      ...
      case "displace":
        mainProps[this.props.side] = this._length;
        drawerProps[this.props.side] =
          -this.getDeviceLength() + this._length + this._offsetOpen;
        break;
    }
    ...
    if (this.main && this.drawer && this.mainOverlay && this.drawerOverlay) {
      ...
      if (drawerOverlayProps)
        this.drawerOverlay.setNativeProps({ style: drawerOverlayProps });
    }
  };

  shouldOpenDrawer(delta) {
    let hasActiveHeading =
      this._open ^ (delta > 0) ^ this.isRightOrBottomSide();
    if (!hasActiveHeading) return this._open;
    else
      return (
        this._open ^
        (Math.abs(delta) > this.getDeviceLength() * this.props.panThreshold)
      );
  }

  onPanResponderTerminate = (e, gestureState) => {
    this._panning = false;
    this.shouldOpenDrawer(this.getGestureDelta(gestureState))
      ? this.open()
      : this.close();
  };
  ...
  onStartShouldSetPanResponderCapture = (e, gestureState) => {
    if (this.shouldCaptureGestures())
      return this.processShouldSet(e, gestureState);
    return false;
  };
  ...
  onPanResponderRelease = (e, gestureState) => {
    let delta = this.getGestureDelta(gestureState);
    this._panning = false;
    if (Date.now() - this._panStartTime < TAP_DURATION)
  ...
  processShouldSet = (e, gestureState) => {
    ...
    if (!this._open && Math.abs(gestureState.dy) >= Math.abs(gestureState.dx))
      return false;
  ...
  processMoveShouldSet = (e, gestureState) => {
    let inMask = this.testPanResponderMask(e, gestureState);
    if (!inMask && (!this.props.acceptPanOnDrawer || this._open === false))
      return false;
    if (!this.props.acceptPan) return false;
    ...
    let swipeOppositeAxis =
      Math.abs(deltaOppositeAxis) >= Math.abs(delta) ? true : false;
    let swipeInCloseDirection = this.isLeftOrTopSide()
      ? swipeToLeftOrTop
      : swipeToRightOrBottom;
    if (
      swipeOppositeAxis ||
      (this._open && !swipeInCloseDirection) ||
      (!this._open && swipeInCloseDirection)
    ) {
      return false;
    }
  ...
  shouldCaptureGestures() {
    ...
    if (this.props.captureGestures === "closed" && this._open === false)
      return true;
    if (this.props.captureGestures === "open" && this._open === true)
      return true;
    return false;
  }
  ...
  testPanResponderMask = (e, gestureState) => {
    ...
    let pos0 = this.isLeftOrRightSide()
      ? e.nativeEvent.pageX
      : e.nativeEvent.pageY;
    let deltaOpen = this.isLeftOrTopSide()
      ? this.getDeviceLength() - pos0
      : pos0;
    let deltaClose = this.isLeftOrTopSide()
      ? pos0
      : this.getDeviceLength() - pos0;
  ...
  /*** DYNAMIC GETTERS ***/
  getDeviceLength = (viewport = this.state.viewport) =>
    this.isLeftOrRightSide() ? viewport.width : viewport.height;
  ...
  getMainWidth = (viewport = this.state.viewport) => {
    return this.isLeftOrRightSide()
      ? viewport.width - this._offsetClosed
      : viewport.width;
  };
  getMainHeight = (viewport = this.state.viewport) => {
    return this.isTopOrBottomSide()
      ? viewport.height - this._offsetClosed
      : viewport.height;
  };
  getDrawerWidth = (viewport = this.state.viewport) => {
    return this.isLeftOrRightSide()
      ? viewport.width - this._offsetOpen
      : viewport.width;
  };
  getDrawerHeight = (viewport = this.state.viewport) => {
    return this.isTopOrBottomSide()
      ? viewport.height - this._offsetOpen
      : viewport.height;
  };
  getOpenMask = (viewport = this.state.viewport) => {
    ...
    if (this.props.panCloseMask)
      return this.getDeviceLength(viewport) * this.props.panCloseMask;
    return Math.max(0.05, this._offsetOpen);
  };
  getClosedMask = () => {
    if (this.props.panOpenMask && this.props.panOpenMask % 1 === 0)
      return this.props.panOpenMask;
    if (this.props.panOpenMask)
      return this.getDeviceLength() * this.props.panOpenMask;
    return Math.max(0.05, this._offsetClosed);
  };
  getOpenOffset = (props, viewport) => {
    if (typeof props.openDrawerOffset === "function")
      return props.openDrawerOffset(viewport);
    return props.openDrawerOffset > 1 || props.openDrawerOffset < 0
      ? props.openDrawerOffset
      : props.openDrawerOffset * this.getDeviceLength(viewport);
  };
  getClosedOffset = (props, viewport) => {
    if (typeof props.closedDrawerOffset === "function")
      return props.closedDrawerOffset(viewport);
    return props.closedDrawerOffset > 1 || props.closedDrawerOffset < 0
      ? props.closedDrawerOffset
      : props.closedDrawerOffset * this.getDeviceLength(viewport);
  };
  getGestureDelta = gestureState =>
    this.isLeftOrRightSide() ? gestureState.dx : gestureState.dy;
  getGestureDeltaOppositeAxis = gestureState =>
    this.isLeftOrRightSide() ? gestureState.dy : gestureState.dx;
  /*** END DYNAMIC GETTERS ***/
  ...
  render() {
    let first =
      this.props.type === "overlay" ? this.renderMain() : this.renderDrawer();
    let second =
      this.props.type === "overlay" ? this.renderDrawer() : this.renderMain();
  ...
  renderMain() {
    return (
      <Animated.View
        ...
        ref={c => (this.main = c)}
        style={[
          this.stylesheet.main,
          { height: this.getMainHeight(), width: this.getMainWidth() },
          this.props.isScale && { transform: [{ scale: this.state.scale }] }
        ]}
      >
        {this.props.children}
        <View
          pointerEvents={
            this._open && this.shouldCaptureGestures() ? "auto" : "none"
          }
  ...
  renderDrawer() {
    return (
      <View
        ...
        style={[
          this.stylesheet.drawer,
          { height: this.getDrawerHeight(), width: this.getDrawerWidth() }
        ]}
      >
        {this.props.content}
        <View
